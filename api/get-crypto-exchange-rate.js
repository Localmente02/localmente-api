...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const coingeckoApiUrl = `https://api.coingecko.com/api/v3/simple/price?ids=${currencyCode.toLowerCase()}&vs_currencies=eur`;

    const response = await fetch(coingeckoApiUrl);
    const data = await response.json();

    // Esempio di risposta: {"bitcoin":{"eur":28000}}
    const cryptoData = data[currencyCode.toLowerCase()];

    if (cryptoData && cryptoData.eur) {
      // Il tasso restituito da CoinGecko è 1 unità crypto = X EUR.
